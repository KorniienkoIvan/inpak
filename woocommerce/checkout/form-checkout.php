	...
	<div class="chechout_fields_wrapper">
		...
		<?php if ( $checkout->get_checkout_fields() ) : ?>
			...
			<div class="col2-set col-7 client_data" id="customer_details">
				<div class="col-12">
  					<div class="chekout_col_title_wrapper"><span class="col-number">2</span><div class="chekout_col_title"><?php if(get_locale() == 'uk'){echo 'Ваші контактні данні';}elseif(get_locale() == 'ru_RU'){echo 'Ваши контактные данные';} else {echo 'Ваші контактні данні';} ?></div></div>
...
if($cart < 3000){
	...
	<script>
		jQuery(document).ready(function(){
			jQuery('#by_courier_field').css('pointer-events', 'none').css('opacity', '0.5');
		});
	</script>
	<?php
}
...
<script>
	jQuery(document).ready(function(){
		...
		jQuery('#shipping_method li:not(:first) label').click(function(){
		...
		jQuery('#shipping_method li:first label').click(function(){
			jQuery('#shipping_method li label').removeClass('checked');
			jQuery(this).addClass('checked');
			jQuery('.wcus-checkout-fields').detach().appendTo(jQuery(this).parent());
		});
