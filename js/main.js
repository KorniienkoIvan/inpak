...
$(document).ready(function(){
  var popup = $('.single-product .add_to_cart_popup_wrapper').length;
  if(popup > 0){
    $('body.single-product').addClass('open-popup--to-cart');
    $('body.woocommerce.archive').addClass('open-popup--to-cart');
  };
});
...
jQuery(document).ready(function(){
  jQuery('.cart-icon').click(function(){
    $('body').addClass('open-popup');
    $('body').addClass('open-popup--to-cart');
  }); 
});
...
$(document).on("click", '.outofstock .out-of-stock-button-wrapper a[href=""]', function(event) {
  event.preventDefault();
  $(this).closest('.product').find('.yith-wcwtl-output').css('display', 'block');
  $(this).closest('.product').find('.out-of-stock-button-wrapper').css('display', 'none');
});
...
$('#yith-wcwtl-output').click(function(event){
  event.stopPropagation();
});

$('.single-product .out-of-stock-button').click(function(event){
  event.stopPropagation();
});


$(document).ready(function(){
  var element_content = $('.woocommerce-error li').html();
  console.log(element_content);
  if(element_content == 'You have already registered for this waiting list'){
    console.log('123');
  }
}); 


$('.wc-proceed-to-checkout a[href="#"]').click(function(e){
  e.preventDefault();
  $('.shipping_price_block').css('border', '3px solid #e30613');
});
